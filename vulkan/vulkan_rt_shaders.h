...
void main() {
    ...
    if (didHit) {
        vec3 baseColor = GetObjectColor(objectID, 0u);
        ...
        // Mirror reflection
            ...
            if (didHit) {
                ...
                if (objectID == OBJ_LIGHT) {
                    ...
                    vec3 toLight = normalize(scene.lightPos - hitPos);
                    float NdotL = max(dot(hitNormal, toLight), 0.0);
                    if ((scene.features & FEATURE_SPOTLIGHT) != 0u) {
                        float reflSpot = SpotlightAttenuation(hitPos - scene.lightPos);
        ...
        // Glass transparency
            ...
            if (didHit) {
                ...
                if (objectID != OBJ_LIGHT) {
                    ...
                    float NdotL = max(dot(hitNormal, toLight), 0.0);
                    if ((scene.features & FEATURE_SPOTLIGHT) != 0u) {
                        float glassSpot = SpotlightAttenuation(hitPos - scene.lightPos);
        ...
        // Diffuse surfaces
            ...
            vec3 toLight = normalize(scene.lightPos - hitPos);
            float NdotL = max(dot(hitNormal, toLight), 0.0);
            float lightDist = length(scene.lightPos - hitPos);

            // Shadow
            float shadow = 1.0;
            // Shadow ray would go here (simplified for now)
            ...
            if ((scene.features & FEATURE_SPOTLIGHT) != 0u) {
                float spotAtten = SpotlightAttenuation(hitPos - scene.lightPos);
                totalAtten *= spotAtten;
            }

            vec3 ambient = baseColor * 0.15 * ao;
            vec3 diffuse = baseColor * NdotL * shadow * totalAtten;

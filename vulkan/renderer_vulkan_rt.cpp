...
static void GenerateCornellBox(std::vector<VkRTVertex>& verts, std::vector<uint32_t>& indices) {
    ...
    // Mirror panel (angled, back-left)
    baseIdx = (uint32_t)verts.size();
    float mx = -0.6f, mz = 0.4f, mw = 0.5f, mh = 0.8f;
    float angle = 0.785398f; // 45 degrees
    float ca = cosf(angle), sa = sinf(angle);
    float3 mnorm = {ca, 0, -sa};
    verts.push_back({mx,      -1,      mz,      mnorm.x, mnorm.y, mnorm.z, 0.95f, 0.95f, 0.95f, 1});
    verts.push_back({mx+mw*ca, -1,     mz-mw*sa, mnorm.x, mnorm.y, mnorm.z, 0.95f, 0.95f, 0.95f, 1});
    verts.push_back({mx+mw*ca, -1+mh,  mz-mw*sa, mnorm.x, mnorm.y, mnorm.z, 0.95f, 0.95f, 0.95f, 1});
    verts.push_back({mx,      -1+mh,   mz,      mnorm.x, mnorm.y, mnorm.z, 0.95f, 0.95f, 0.95f, 1});
    indices.push_back(baseIdx+0); indices.push_back(baseIdx+1); indices.push_back(baseIdx+2);
    indices.push_back(baseIdx+0); indices.push_back(baseIdx+2); indices.push_back(baseIdx+3);
...
bool InitVulkanRT(HWND hwnd) {
    ...
    if (vkCreateInstance(&instanceInfo, nullptr, &s_instance) != VK_SUCCESS) {
        ...
        MessageBoxA(hwnd, "Failed to create Vulkan instance.\nPlease ensure Vulkan drivers are installed.",
                   "Vulkan RT Error", MB_OK | MB_ICONERROR);
        return false;
    }
    ...
    if (vkCreateDevice(s_physicalDevice, &deviceInfo, nullptr, &s_device) != VK_SUCCESS) {
        ...
        MessageBoxA(hwnd, "Failed to create Vulkan device with RT extensions.\nYour GPU may not support all required features.",
                   "Vulkan RT Error", MB_OK | MB_ICONERROR);
        return false;
    }
    ...
    // Load RT extension functions
    if (!LoadRTExtensions()) {
        MessageBoxA(hwnd, "Failed to load ray tracing extension functions.", "Vulkan RT Error", MB_OK | MB_ICONERROR);
        return false;
    }
    ...
    if (!CreateGeometryBuffers()) {
        MessageBoxA(hwnd, "Failed to create geometry buffers.", "Vulkan RT Error", MB_OK | MB_ICONERROR);
        CleanupVulkanRT();
        return false;
    }
    ...
    if (!CreateBLAS(s_staticVertexBuffer, s_staticIndexBuffer, s_staticVertexCount, s_staticIndexCount,
        ...
        MessageBoxA(hwnd, "Failed to create static BLAS.", "Vulkan RT Error", MB_OK | MB_ICONERROR);
        CleanupVulkanRT();
        return false;
    }
    ...
    if (!CreateBLAS(s_cubesVertexBuffer, s_cubesIndexBuffer, s_cubesVertexCount, s_cubesIndexCount,
        ...
        MessageBoxA(hwnd, "Failed to create cubes BLAS.", "Vulkan RT Error", MB_OK | MB_ICONERROR);
        CleanupVulkanRT();
        return false;
    }
    ...
    // ========== Step 11: Create TLAS ==========
    if (!CreateTLAS()) {
        MessageBoxA(hwnd, "Failed to create TLAS.", "Vulkan RT Error", MB_OK | MB_ICONERROR);
        CleanupVulkanRT();
        return false;
    }
    ...
    if (!CreateOutputImage()) {
        MessageBoxA(hwnd, "Failed to create output image.", "Vulkan RT Error", MB_OK | MB_ICONERROR);
        CleanupVulkanRT();
        return false;
    }
    ...
    if (!CreateUniformBuffer()) {
        MessageBoxA(hwnd, "Failed to create uniform buffer.", "Vulkan RT Error", MB_OK | MB_ICONERROR);
        CleanupVulkanRT();
        return false;
    }
    ...
    if (!CreateDescriptorSetLayout()) {
        MessageBoxA(hwnd, "Failed to create descriptor set layout.", "Vulkan RT Error", MB_OK | MB_ICONERROR);
        CleanupVulkanRT();
        return false;
    }
    ...
    if (!CreateDescriptorSet()) {
        MessageBoxA(hwnd, "Failed to create descriptor set.", "Vulkan RT Error", MB_OK | MB_ICONERROR);
        CleanupVulkanRT();
        return false;
    }
    ...
    if (!CreateRTPipeline()) {
        MessageBoxA(hwnd, "Failed to create ray tracing pipeline.", "Vulkan RT Error", MB_OK | MB_ICONERROR);
        CleanupVulkanRT();
        return false;
    }
    ...
    if (!CreateShaderBindingTable()) {
        MessageBoxA(hwnd, "Failed to create shader binding table.", "Vulkan RT Error", MB_OK | MB_ICONERROR);
        CleanupVulkanRT();
        return false;
    }
